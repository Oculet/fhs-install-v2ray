...
# Modify ........: No:9132 04/07/08 By Mandy aimf300.4gl 第185 OUTER azf_file,where 條件azf01請改為azf_file.azf01,否則報表OUTER會有問題,實際是修正aimf300.ora
...
DEFINE tm  RECORD                            # Print condition RECORD
          #wc      LIKE type_file.chr1000,   # Where condition  #No.FUN-690026 VARCHAR(500)#TQC-CC0031 mark
           ...
           d       LIKE type_file.chr1,      #FUN-E50039 add 
...
#C20122301-14413 add by qiuwl ---start---                      
DEFINE l_timestamp          LIKE type_file.chr30
DEFINE l_sessionid          LIKE type_file.chr30
#C20122301-14413 add by qiuwl ---end---
...
MAIN
   ...
   #C20122301-14413 add by qiuwl ---start---
   CALL cl_fr_get_sid_timestamp() RETURNING l_sessionid ,l_timestamp
   LET g_fr_sessionid = l_sessionid
   LET g_fr_timestamp = l_timestamp
   #C20122301-14413 add by qiuwl ---end---
...
#No.FUN-860026---begin
   ...
   #LET l_table1 = cl_prt_temptable('aimf3001',g_sql) CLIPPED  #C20122301-14413 mark
   LET l_table1 = cl_fr_temptable('aimf3001',g_sql,'') CLIPPED  #C20122301-14413 add
   IF  l_table1 = -1 THEN 
      EXIT PROGRAM 
   END IF 
   #FUN-E50039--add--str--
   LET g_sql="inbc01.inbc_file.inbc01,inbc03.inbc_file.inbc03,",
             "inbc05.inbc_file.inbc05,inbc05_name.agd_file.agd03,",  
             "inbc06.inbc_file.inbc06,inbc06_name.agd_file.agd03,", 
             "inbc07.inbc_file.inbc07,inbc07_1.inbc_file.inbc07,",
             "inbc07_2.inbc_file.inbc07,inbc07_3.inbc_file.inbc07,",
             "inbc07_4.inbc_file.inbc07,agd04_color.agd_file.agd04,",
             "agd04_size.agd_file.agd04"
   #LET l_table3 = cl_prt_temptable('aimf3003',g_sql) CLIPPED  #C20122301-14413 mark
   LET l_table3 = cl_fr_temptable('aimf3003',g_sql,'') CLIPPED  #C20122301-14413 add
   IF l_table3 = -1 THEN
   ...
   LET tm.d=ARG_VAL(14)           #FUN-E50039  add
   LET g_rep_user = ARG_VAL(10)
   LET g_rep_clas = ARG_VAL(11)
#   LET g_template = ARG_VAL(12)  #C21110801-16031 by zhongyl mark
   LET g_template = NULL  #C21110801-16031 by zhongyl add
  ...
  #TQC-610072-end
      THEN CALL aimf300_tm(0,0)             # Input print condition
     # ELSE LET tm.wc="ina01= '",tm.wc CLIPPED,"'"     #TQC-610072
    ELSE                                               #TQC-610072
           CALL aimf300()                   # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
END MAIN
 
FUNCTION aimf300_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,    #No.FUN-690026 SMALLINT
   ...
   OPEN WINDOW aimf300_w AT p_row,p_col
        WITH FORM "aim/42f/aimf300"
################################################################################
...
WHILE TRUE
   ...
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW aimf300_w 
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
   ...
   END IF
   INPUT BY NAME tm.c,tm.a,tm.b,tm.more,tm.d WITHOUT DEFAULTS    #No.FUN-860026 add tm.c  #FUN-E50039  add tm.d
         #No.FUN-580031 --start--
         BEFORE INPUT
...
################################################################################
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('aimf300',g_time,l_cmd)    # Execute cmd at later time
      END IF
      CLOSE WINDOW aimf300_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690115 BY dxfwo 
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#FUN-E50039--add--str--
 DEFINE       l_inbc05       LIKE inbc_file.inbc05,
              l_inbc05_name  LIKE agd_file.agd03,    #顏色
              l_inbc06       LIKE inbc_file.inbc06,
              l_inbc06_name  LIKE agd_file.agd03,    #尺寸
              l_inbc07       LIKE inbc_file.inbc07,
              l_inbc07_1     LIKE inbc_file.inbc07,  #數量(1~10)
              l_inbc07_2     LIKE inbc_file.inbc07,  #數量(11~20)
              l_inbc07_3     LIKE inbc_file.inbc07,  #數量(21~30)
              l_inbc07_4     LIKE inbc_file.inbc07   #數量(31~40)
...
###No.FUN-940043 START###
     ...
     DEFINE   l_inb902       LIKE inb_file.inb902    #C21120401-16508 
     DEFINE   l_inb904_1     LIKE inb_file.inb904    #C21120401-16508
     DEFINE   l_inb905       LIKE inb_file.inb905    #C21120401-16508
     DEFINE   l_inb907_5     LIKE inb_file.inb907    #C21120401-16508
...
#No.FUN-860026---end      
     ...
     CALL cl_del_data(l_table3)  #FUN-E50039 add
...
#No.FUN-860026---begin
    ...
    END IF            
     LET l_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table1 CLIPPED,                                                               
                 #" VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)"  #C20122301-14413 mark
                 " VALUES(",l_sessionid,",to_timestamp('",l_timestamp,"','yyyy-mm-dd hh24-mi-ss.ff'),?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)"  #C20122301-14413 add
...
#                "       ina06,ina07,inb03,inb04,ima02,inb05,inb06,",          #MOD-840423-modify
     ...
     PREPARE aimf300_prepare1 FROM l_sql
     ...
     END IF
     DECLARE aimf300_curs1 CURSOR FOR aimf300_prepare1

#FUN-E50039--add--str--
  #抓取尺寸總共有幾個
   LET g_sql="SELECT DISTINCT(inbc06)",  
             "  FROM inbc_file",
             " WHERE inbc01=? AND inbc03=? ",
             " ORDER BY inbc06" 
   PREPARE f300_inbc_pr FROM g_sql
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare f300_inbc_pr:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
   END IF
   DECLARE f300_inbc_cs CURSOR FOR f300_inbc_pr

   #抓取該尺寸的顏色總共有幾個
   LET g_sql="SELECT inbc05,inbc07",
             "  FROM inbc_file",
             " WHERE inbc01=? AND inbc03=? AND inbc06=? ",
             "   AND inbc07 <> '0'",
             " ORDER BY inbc06,inbc07"
   PREPARE f300_inbc_pr1 FROM g_sql
   IF SQLCA.sqlcode THEN
      CALL cl_err('prepare f300_inbc_pr1:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
   END IF
   DECLARE f300_inbc_cs1 CURSOR FOR f300_inbc_pr1
#FUN-E50039--add--end
...
#    PREPARE f300_pr1 FROM l_sql_1
#    IF SQLCA.sqlcode THEN
#       CALL cl_err('prepare f300_pr1:',SQLCA.sqlcode,0)
#       CALL cl_used(g_prog,g_time,2) RETURNING g_time
#       EXIT PROGRAM
#    END IF
#    DECLARE f300_cs1 CURSOR FOR f300_pr1
###No.FUN-940043 END###
#No.TQC-C10034 ----- mark ----- end
   #FUN-710084  --begin
   # CALL cl_outnam('aimf300') RETURNING l_name
     #No.FUN-580005 --start--
   ...
   # START REPORT aimf300_rep TO l_name
   #FUN-710084  --end
 
     LET g_pageno = 0
     FOREACH aimf300_curs1 INTO sr.*
       IF SQLCA.sqlcode != 0 THEN
...
#No.FUN-860026---begin       
    ...
    END FOREACH                                                                                                                     
      ...
      END FOR                                                                                                                       
#No.FUN-860026---end     
        LET l_inb902 = sr.inb902   #C21120401-16508 add
        LET l_inb904_1 = sr.inb904 #C21120401-16508 add
        LET l_inb905 = sr.inb905   #C21120401-16508 add
        LET l_inb907_5 = sr.inb907 #C21120401-16508 add
...
#                                 sr.ina06,sr.ina07,sr.inb03,sr.inb04,sr.ima02,              #MOD-840423-modify
      ...
      IF tm.d = 'Y' THEN
         #抓取顏色尺寸資料
         LET l_next = 1 
         FOREACH f300_inbc_cs USING sr.ina01,sr.inb03 INTO l_inbc06
            IF SQLCA.sqlcode != 0 THEN
               CALL cl_err('foreach f300_inbc_cs:',SQLCA.sqlcode,1) EXIT FOREACH
            END IF

            #尺寸的名稱  
            LET l_inbc05_name=''
            LET l_agd04_size=''
            SELECT agd03,agd04 INTO l_inbc06_name,l_agd04_size FROM agd_file
             WHERE agd02=l_inbc06
               AND agd01=(SELECT ima941 FROM ima_file WHERE ima01=sr.inb04)
            LET l_agd04_color=''
            LET l_inbc07=''
            CASE
               WHEN (l_next <= 10)  #1~10
                  ...
                  FOREACH f300_inbc_cs1 USING sr.ina01,sr.inb03,l_inbc06
                                   INTO l_inbc05,l_inbc07
                     LET l_inbc07_1=''
                     LET l_inbc07_2=''
                     LET l_inbc07_3=''
                     LET l_inbc07_4=''
                     LET l_inbc07_1=l_inbc07
                     LET l_inbc05_name=''
                     LET l_agd04_color=''
                     SELECT agd03,agd04 INTO l_inbc05_name,l_agd04_color FROM agd_file
                      WHERE agd02=l_inbc05
                        AND agd01=(SELECT ima940 FROM ima_file WHERE ima01=sr.inb04)
                     EXECUTE insert_prep3 USING sr.ina01,sr.inb03,
                                                l_inbc05,l_inbc05_name,l_inbc06,
                                                l_inbc06_name,l_inbc07,l_inbc07_1,
                                                l_inbc07_2,l_inbc07_3,l_inbc07_4,
               ...
               WHEN (l_next > 30) #30以上
                  ...
                  FOREACH f300_inbc_cs1 USING sr.ina01,sr.inb03,l_inbc06
                                   INTO l_inbc05,l_inbc06
                     LET l_inbc07_3=''
                     LET l_inbc07_4=l_inbc07
                     LET l_inbc05_name=''
                     LET l_agd04_color=''
                     SELECT agd03,agd04 INTO l_inbc05_name,l_agd04_color FROM agd_file
                      WHERE agd02=l_inbc05
...
#FUN-E50039--add--end
   ...
   # LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED, l_table CLIPPED #No.FUN-820026
     #LET l_sql = "SELECT * FROM ",g_cr_db_str CLIPPED, l_table CLIPPED,"|",  #C20122301-14413 mark
                 #"SELECT * FROM ",g_cr_db_str CLIPPED, l_table1 CLIPPED,"|",  #No.FUN-860026  #C20122301-14413 mark
                 #"SELECT * FROM ",g_cr_db_str CLIPPED,l_table3 CLIPPED        #FUN-E50039 add  #C20122301-14413 amrk
     LET g_str = g_sma115
...
#        LET g_str = g_str ,";",tm.wc    #No.FUN-860026 
     ...
     LET g_str = g_str,";",l_str1,";",tm.c   #No.FUN-860026
                 ...
                 ,";",g_prog  #C20122301-14413 add

   # CALL cl_prt_cs3('aimf300',l_sql,g_str)   #TQC-730088
#No.TQC-C10034 ----- mark ----- begin
###No.FUN-940043 START###
...
#No.TQC-C10034 ----- mark ----- end
     ...
     #CALL cl_prt_cs3('aimf300','aimf300',l_sql,g_str)  #C20122301-14413 mark
     CALL cl_fr_view(g_str)  #C20122301-14413 add
### FUN-710084 End ###
END FUNCTION
### FUN-710084 Start ###
{
REPORT aimf300_rep(sr)
